...
def agent_e_transfer_node(state: AgentEInput, config: RunnableConfig) -> AgentEOutput:
    """迁移应用智能体"""
    ...
    if result_json:
        return AgentEOutput(
            agent_e_response=result_json.get("response", response_text),
            agent_e_transfer_tasks=result_json.get("transfer_tasks", []),
            agent_e_guidance=result_json.get("guidance", "")
        )
    else:
        return AgentEOutput(agent_e_response=response_text)
...
def merge_results_node(state: MergeNodeInput, config: RunnableConfig) -> MergeNodeOutput:
    ...
    return MergeNodeOutput(
        ...
        agent_b_concept_diagram=state.agent_b_concept_diagram,
